...
struct ioc4_misc_regs {
	...
	union ioc4_pci_err_addr_l {
		uint32_t raw;
		struct {
			uint32_t valid;	/* Address captured */
			uint32_t master_id;	/* Unit causing error
						 ...
						 * 9-15: Undefined
						 */
			uint32_t mul_err;	/* Multiple errors occurred */
			uint32_t addr;	/* Bits 31-6 of error addr */
		} fields;
	} pci_err_addr_l;
	...
	union ioc4_sio_int {
		uint32_t raw;
		struct {
			uint8_t tx_mt;	/* TX ring buffer empty */
			uint8_t rx_full;	/* RX ring buffer full */
			uint8_t rx_high;	/* RX high-water exceeded */
			uint8_t rx_timer;	/* RX timer has triggered */
			uint8_t delta_dcd;	/* DELTA_DCD seen */
			uint8_t delta_cts;	/* DELTA_CTS seen */
			uint8_t intr_pass;	/* Interrupt pass-through */
			uint8_t tx_explicit;	/* TX, MCW, or delay complete */
		} fields[4];
	} sio_ir;		/* Serial interrupt state */
	union ioc4_other_int {
		uint32_t raw;
		struct {
			uint32_t ata_int;	/* ATA port passthru */
			uint32_t ata_memerr;	/* ATA halted by mem error */
			uint32_t memerr;	/* Serial halted by mem err */
			uint32_t kbd_int;	/* kbd/mouse intr asserted */
			uint32_t reserved;	/* zero */
			uint32_t rt_int;	/* INT_OUT section latch */
			uint32_t gen_int;	/* Intr. from generic pins */
		} fields;
	...
	union ioc4_sio_cr {
		uint32_t raw;
		struct {
			uint32_t cmd_pulse;	/* Bytebus strobe width */
			uint32_t arb_diag;	/* PCI bus requester */
			uint32_t sio_diag_idle;	/* Active ser req? */
			uint32_t ata_diag_idle;	/* Active ATA req? */
			uint32_t ata_diag_active;	/* ATA req is winner */
			uint32_t reserved;	/* zero */
		} fields;
	} sio_cr;
	uint32_t unused1;
	union ioc4_int_out {
		uint32_t raw;
		struct {
			uint32_t count;	/* Period control */
			uint32_t mode;	/* Output signal shape */
			uint32_t reserved;	/* zero */
			uint32_t diag;	/* Timebase control */
			uint32_t int_out;	/* Current value */
		} fields;
	...
	union ioc4_gpcr {
		...
		struct {
			...
			uint32_t edge;	/* Edge/level mode */
			uint32_t reserved1;	/* zero */
			uint32_t int_out_en;	/* INT_OUT enable */
	...
	union ioc4_gpdr {
		uint32_t raw;
		struct {
			uint32_t gen_pin;	/* State of pins */
			uint32_t reserved;
		} fields;
	...
	union ioc4_gppr {
		uint32_t raw;
		struct {
			uint32_t gen_pin;	/* Single pin state */
			uint32_t reserved;
		} fields;
